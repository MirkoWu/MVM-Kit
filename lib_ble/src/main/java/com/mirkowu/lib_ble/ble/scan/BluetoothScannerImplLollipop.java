package com.mirkowu.lib_ble.ble.scan;
...
class BluetoothScannerImplLollipop extends BleScannerCompat {
    ...
    private final ScanCallback scannerCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            if (BleManager.options().isParseScanData){
    ...
    private void setScanSettings() {
        boolean background = Utils.isBackground(BleManager.getInstance().getContext());
        BleLog.d(TAG, "currently in the background:>>>>>"+background);

        ScanFilter filter = BleManager.options().getScanFilter();
        if (filter != null){
            filters.add(filter);
        }
        if (background){
            UUID uuidService = BleManager.options().getUuidService();
            if (filter == null){
