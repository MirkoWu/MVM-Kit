...
public class ScanRecord implements Parcelable {
    ...
    public void readFromParcel(Parcel source) {
        this.mAdvertiseFlags = source.readInt();
        this.mServiceUuids = source.createTypedArrayList(ParcelUuid.CREATOR);
        this.mManufacturerSpecificData = source.readSparseArray(byte[].class.getClassLoader());
        int mServiceDataSize = source.readInt();
        this.mServiceData = new HashMap<ParcelUuid, byte[]>(mServiceDataSize);
        for (int i = 0; i < mServiceDataSize; i++) {
            ParcelUuid key = source.readParcelable(ParcelUuid.class.getClassLoader());
            byte[] value = source.createByteArray();
            this.mServiceData.put(key, value);
        }
        this.mTxPowerLevel = source.readInt();
        this.mDeviceName = source.readString();
        this.mBytes = source.createByteArray();
    }
