...
import com.mirkowu.lib_ble.BuildConfig;
...
import com.mirkowu.lib_ble.ble.utils.ByteUtils;
import com.mirkowu.lib_ble.ota.OtaListener;
...
public final class BleRequestImpl<T extends BleDevice> {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    ...
                    if (connectWrapperCallback != null) {
                        ...
                        connectWrapperCallback.onConnectionChanged(bleDevice, BleDevice.CONNECTED);
                    }
                    ...
                    if (connectWrapperCallback != null) {
                        ...
                        connectWrapperCallback.onConnectionChanged(bleDevice, BleDevice.DISCONNECT);
                    }
    ...
    void initialize(Context context) {
        ...
        this.options = BleManager.options();
    ...
    public boolean connect(final T bleDevice) {
        ...
        if (connectedAddressList.contains(bleDevice.getAddress()) && bleDevice.isConnected()) {
        ...
        connectWrapperCallback.onConnectionChanged(bleDevice, BleDevice.CONNECTING);
    ...
    public boolean isDeviceBusy(T device) {
        boolean state = false;
        try {
            BluetoothGatt gatt = getBluetoothGatt(device.getAddress());
            if (gatt != null) {
    ...
    private void displayGattServices(BluetoothGatt gatt) {
        ...
        if (null != connectWrapperCallback) {
            connectWrapperCallback.onConnectSuccess(getBleDeviceInternal(device.getAddress()));
        }
    }
