...
import com.mirkowu.lib_ble.BLEManager
    ...
    private fun startLeScan() {
        ...
        if (bleManager.isSupportBLE(context)) {
            PermissionsUtil.getInstance()
                .requestPermissions(this, arrayOf(Manifest.permission.ACCESS_COARSE_LOCATION,
                            ...
                            if (bleManager.isEnable()) {
                                val scanning = bleManager.startLeScan { device, rssi, scanRecord ->
                                    //type=1传统蓝牙  2BLE 3 双模蓝牙 0未知
                                     if (!mAdapter.getData().contains(device) && device.type >= 2) {
